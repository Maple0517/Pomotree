...
describe("stats selectors", () => {
  it("counts completed and partial today but not discarded", () => {
    ...
    expect(getTodayStats(sessions, interruptions, new Date("2026-05-25T12:00:00.000Z"))).toEqual({
      completedCount: 1,
      partialCount: 1,
  ...
  it("uses one local-day boundary for sessions and interruptions", () => {
    ...
    const sessions = [
      { ...makeSession("previous-day", "completed", "parent", 1500), startedAt: previousLocalDay },
      { ...makeSession("today", "completed", "parent", 600), startedAt: todayStart },
      { ...makeSession("next-day", "partial", "parent", 300), startedAt: nextLocalDay },
    ];
    const interruptions: Interruption[] = [
    ...
    expect(getTodayStats(sessions, interruptions, new Date(2026, 4, 25, 12, 0, 0))).toEqual({
      completedCount: 1,
      partialCount: 0,
